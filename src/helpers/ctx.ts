...
import {UserService} from "../club/user/user.service";
...
export async function isUserAdmin(ctx, users: UserService) {
  let isAdmin = false;
  ...
  try {
    ...
    if (admins.find(o => parseInt(o.user_id) === parseInt(userid))) {
      isAdmin = true
    }
  } catch (e) {
